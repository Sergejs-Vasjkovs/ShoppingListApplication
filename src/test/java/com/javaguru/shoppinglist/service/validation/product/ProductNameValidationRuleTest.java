...
public class ProductNameValidationRuleTest {
    ...
    public void shouldThrow_ProductNameValidationException_NameLengthToShort() {
        product.setName(RandomStringUtils.randomAlphabetic(MIN_PRODUCT_NAME_LENGTH - 1));
        expectedException.expect(ProductValidationException.class);
        expectedException.expectMessage("Product name minimum length is " + MIN_PRODUCT_NAME_LENGTH +
    ...
    public void shouldThrow_ProductNameValidationException_NameLengthToLong() {
        product.setName(RandomStringUtils.randomAlphabetic(MAX_PRODUCT_NAME_LENGTH + 1));
        expectedException.expect(ProductValidationException.class);
        expectedException.expectMessage("Product name minimum length is " + MIN_PRODUCT_NAME_LENGTH +
                " characters, maximum is " + MAX_PRODUCT_NAME_LENGTH);
        victim.validate(product);
    ...
    public void shouldValidate_WithoutException() {
        product.setName(RandomStringUtils.randomAlphabetic(MAX_PRODUCT_NAME_LENGTH));
        victim.validate(product);
    }
